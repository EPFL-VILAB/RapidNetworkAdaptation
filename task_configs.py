...
class RealityTask(Task):
    ...
    def __init__(self, name, dataset, tasks=None, use_dataset=True, shuffle=False, batch_size=64):
        ...
        loader = torch.utils.data.DataLoader(
            ...
            num_workers=8, shuffle=self.shuffle, pin_memory=True
        )
...
class ImageTask(Task):
    ...
    def norm(self, pred, target, batch_mean=True, compute_mask=1, compute_mse=True, mask=None):
        # breakpoint()
        if compute_mask:
            if mask is None: mask = ImageTask.build_mask(target, val=self.mask_val)
    ...
    def nll(self, pred, target, batch_mean=True, compute_mask=1, mask=None):
        if compute_mask:
            if mask is None: mask = ImageTask.build_mask(target, val=self.mask_val)
...
def sparsify(x, max_pct=0.015, mask_val=1.):
    x = clamp_maximum_transform(x, max_val=8000.0)
    p = random.uniform(0,max_pct)
    mask = torch.FloatTensor(x[:1].size()).uniform_() < p
    mask = mask.repeat(x.size(0),1,1)
    gt_valid_mask = x < mask_val
    x = x * mask * gt_valid_mask
    return x.float()
...
tasks = [
    ImageTask('rgb'),
    ImageTask('gray'),
    ImageTask('mask_valid', file_name_alt="mask_valid"),
    ImageTask('imagenet', mask_val=0.0),
    ImageTask('sparse_normal',
        file_name_alt="normal", 
        mask_val=0.502,
        transform=partial(sparsify, max_pct=0.015),),
    ImageTask('sparse_depth',
        file_name_alt="depth_zbuffer", 
        transform=partial(sparsify, max_pct=0.01, mask_val=1.),),
