...
def load_train_val(train_tasks, val_tasks=None, fast=False,
    ...
    # tasks = [t.name for t in train_tasks] #+ ['mask_valid']
...
def load_ood(tasks=[tasks.rgb], ood_path=None, sample=28):
...
class TaskDataset(Dataset):
    ...
    def __init__(self, buildings, tasks=[get_task("rgb"), get_task("normal")], data_dirs=DATA_DIRS,
        ...
        filtered_files = None
        exc_list = ['mask_valid','rgb_t1']
        task = [t for t in tasks if t.name not in exc_list][0]
        # task = tasks[0] if tasks[0].name!='mask' else tasks[1]
        task_files = []
    ...
    def convert_path(self, source_file, task):
        ...
        dest_file = f"{building}_{task.file_name_alt}/{task.file_name_alt}/{view}_domain_{task.file_name_alt}.{task.file_ext}"
        if f"{building}_{task.file_name_alt}" not in self.file_map:
            print (f"{building}_{task.file_name_alt} not in file map")
            # IPython.embed()
            return ""
        data_dir = self.file_map[f"{building}_{task.file_name_alt}"]        
